...
Then(
  ...
  async ({ page }) => {
    const userMsgs = page.locator(".ChatPanel__message_user");
    const aiMsgs = page.locator(".ChatPanel__message_designer");
...
Then("the message appears in the chat", async ({ page }) => {
  const userMessages = page.locator(".ChatPanel__message_user");
  await expect(async () => {
...
When("a new message is added", async ({ page }) => {
  await page.fill(".ChatPanel__input", "auto-scroll test message");
  await page.click(".ChatPanel__send");
...
When("the user sends a new improvement", async ({ page }) => {
  await page.fill(".ChatPanel__input", "improve context test");
  await page.click(".ChatPanel__send");
  await page.waitForTimeout(2_000);
});
...
Then("the user cannot send a new message", async ({ page }) => {
  ...
  const btnDisabled = await page
    .locator(".ChatPanel__send")
    .isDisabled({ timeout: 1_000 })
    .catch(() => false);
  expect(isDisabled || btnDisabled).toBe(true);
});
...
Then("sending is enabled again", async ({ page }) => {
  ...
  const sendDisabled = page.locator(".ChatPanel__send_disabled");
  const stillDisabled = await sendDisabled
...
Then("the user cannot send a message", async ({ page }) => {
  const sendDisabled = page.locator(".ChatPanel__send_disabled");
  await expect(sendDisabled).toBeVisible({ timeout: 5_000 });
});

When("the user types text", async ({ page }) => {
  await page.fill(".ChatPanel__input", "some text");
});
...
Given("the user has typed an improvement", async ({ page, world }) => {
  ...
  await page.fill(".ChatPanel__input", "keyboard shortcut test");
});
...
When(
  ...
  async ({ page }) => {
    ...
    await page.locator(".ChatPanel__input").press(`${modifier}+Enter`);
  },
);
...
Given(
  ...
  async ({ page, world }) => {
    ...
    await page.fill(".ChatPanel__input", "first improvement");
    await page.click(".ChatPanel__send");
  },
);
...
When("the user sends another improvement", async ({ page }) => {
  ...
  await expect(async () => {
    const isDisabled = await page
      .locator(".ChatPanel__send_disabled")
      .isVisible()
      .catch(() => false);
    expect(isDisabled).toBe(false);
  }).toPass({ timeout: 120_000 });

  await page.fill(".ChatPanel__input", "second improvement");
  await page.click(".ChatPanel__send");
});
...
When('the user switches to "settings" mode', async ({ page }) => {
  await page
    .locator(".MainLayout__switcher-item", { hasText: /settings/i })
    .click();
});
...
Then(
  ...
  async ({ page }) => {
    // Settings mode should be active
    await expect(
      page.locator(".MainLayout__switcher-item_active", {
        hasText: /settings/i,
      }),
...
When('the user clicks "overview"', async ({ page }) => {
  const overviewItem = page.locator(
    ".DesignSystemModal__menu-item, [class*='menu-item']",
    { hasText: "Overview" },
  );
