...
RSpec.describe "Components API", type: :request do
  ...
  describe "POST /api/components/:id/reimport" do
    it "re-imports a single component from Figma" do
      comp = components(:divider)
      ...
      post "/api/components/#{comp.id}/reimport", headers: auth_headers(user)

      expect(response).to have_http_status(:ok)
      json = JSON.parse(response.body)
      expect(json["status"]).to eq("imported")
    end
  end

  describe "POST /api/component-sets/:id/reimport" do
    it "re-imports a single component set from Figma" do
      ...
      post "/api/component-sets/#{cs.id}/reimport", headers: auth_headers(user)

      expect(response).to have_http_status(:ok)
      json = JSON.parse(response.body)
      expect(json["name"]).to eq("Button")
      expect(json["variants_count"]).to eq(cs.variants.count)
    end
  end
